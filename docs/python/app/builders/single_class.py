...
class SingleClass(Builder):
    ...
    def build(self):
        section_name = clearskies.functional.string.title_case_to_snake_case(self.parent if self.parent else self.title).replace("_", "-").replace(" ", "")
        ...
        docs = self.extract_attribute_docs(
            source_class,
            list(arguments.keys()),
            additional_attribute_sources=[self.classes.find(f"import_path={source}") for source in self.additional_attribute_sources],
        )
        ...
        output_file = section_folder_path / ("index.md" if not self.parent else clearskies.functional.string.title_case_to_snake_case(self.title.replace(" ", "")) + ".md")
