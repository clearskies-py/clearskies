...
class SingleClass(Builder):
    ...
    def build(self):
        section_name = clearskies.functional.string.title_case_to_snake_case(self.title).replace("_", "-").replace(" ", "")
        ...
        class_doc = self.build_header(self.title, title_snake_case, section_name, False, self.nav_order, False)
        ...
        output_file = section_folder_path / "index.md"
