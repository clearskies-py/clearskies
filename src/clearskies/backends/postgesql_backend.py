...
class PostgresqlBackend(cursor_backend.CursorBackend):
    """
    The cursor backend connects your models to a PostgreSQL database.

    ## Installing Dependencies

    clearskies uses psycopg3 to manage the database connection and make queries.  This is not installed by default,
    but is a named extra that you can install when needed via:

    ```bash
    pip install clear-skies[postgresql]
    ```
    ...
    ```python
    class PostgresqlSettings(clearskies.di.AdditionalConfig):
        provide_postgresql_cursor(self, secrets):
            return clearskies.cursors.Postgresql(
    ...
    wsgi = clearskies.contexts.Wsgi(
        some_application,
        additional_configs=[PostgresqlSettings()],
        bindings={
    ...
    class UserPreference(clearskies.Model):
        id_column_name = "id"
        backend = clearskies.backends.PostgresqlBackend(table_prefix="configuration_")
